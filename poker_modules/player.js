 ...
 */
var Player = function( socket, uid, name, chips, admin ) {
	this.public = {
        ...
        // If a user is all in
        allIn: false,
        // End of round, how much player won
        winnings: 0,
        // If they are waiting to sit in
        waitingToSitIn: false,
	...
	this.chips = parseFloat(parseFloat(chips).toFixed(2));
	// The room that send the table events to the player
	this.room = null;
	// Is set to null if the player is not sitting on any tables, otherwise it's set to the table id
    ...
    this.evaluatedHand = {};
    // Option to play a round 1 = wait for big blind, 2 = get in on next round
    this.seatOption = null, 
    // If the user should sit out next big blind
    this.sitOutBigBlind = false;
    // If the user should sit out next hand
    this.sitOutNextHand = false;
    // Time user has in their time bank
    this.timeBank = null;
    // User ID used for updating DB information
    this.uid = uid;
    // Amount of chips to deposit to user at end of round
    this.depositChips = null;
    this.admin = admin;
}
 ...
 * @param  number   chips   
 */
Player.prototype.sitOnTable = function( tableId, seat, chips, timeBank ) {
...
Player.prototype.bet = function( amount ) {
    ...
    if(this.public.chipsInPlay == 0) {
        this.public.allIn = true;
    }
}
...
Player.prototype.evaluateHand = function( board ) {
    ...
    var cardNames = { 'A': 'ace', 'K': 'king', 'Q': 'queen', 'J': 'jack', 'T': 'ten', '9': 'nine', '8': 'eight', '7': 'seven', '6': 'six', '5': 'five', '4': 'four', '3': 'three', '2': 'two' }
	...
	switch( evaluatedHand.rank ) {
		case 'high card':
			...
			break;
		case 'pair':
            evaluatedHand.name = 'pair of ' + getCardName( evaluatedHand.cards[0][0], true );
		...
		case 'three of a kind':
			...
			break;
		case 'straight':
            evaluatedHand.name = 'straight to ' + getCardName( straight[0][0] );
			evaluatedHand.rating = rateHand( evaluatedHand.cards ) + 4000000;
			break;
		case 'flush':
            evaluatedHand.name = 'flush, ' + getCardName( evaluatedHand.cards[0][0] ) + ' high';
			evaluatedHand.rating = rateHand( evaluatedHand.cards ) + 5000000;
			break;
		case 'full house':
			evaluatedHand.name = 'full house, ' + getCardName( evaluatedHand.cards[0][0], true ) + ' full of ' + getCardName( evaluatedHand.cards[3][0], true );
		...
		case 'four of a kind':
			...
			break;
		case 'straight flush':
			evaluatedHand.name = 'straight flush, ' + getCardName( evaluatedHand.cards[4][0] ) + ' to ' + getCardName( evaluatedHand.cards[0][0] );
			evaluatedHand.rating = rateHand( evaluatedHand.cards ) + 8000000;
			break;
		case 'royal flush':
			evaluatedHand.name = 'royal flush';
			evaluatedHand.rating = rateHand( evaluatedHand.cards ) + 8000000;
			break;
	}
